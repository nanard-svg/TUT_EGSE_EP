...
#COnstante égale à 4096 pour bit file utilisé par Aurian, et 256 pour le bit file réellement utilisé
...
####################################################################################################      
    ...
    def creat_menu(self):
        ...
        settings_menu.add_command(label="Set Rising Pulse (0 to 4096)", underline=4, accelerator= "CTRL+R", command= lambda: self.rising_pulse_TH_tk(self.valeur))
        settings_menu.add_command(label="Set Falling Pulse (0 to 4096)", underline=4, accelerator= "CTRL+L", command= lambda: self.rising_pulse_TL_tk(self.valeur))
...
#               #               #               fonction permettant la mise a jours des graphiques              #               #     
    ...
    def update_plot(self, spectre,spectre1):
        ...
        x_range_max =int(min(4095, xlim[1]))
...
class Application:
    ...
    def delay_end(self, fig):    
       self.root.after(25, lambda: self.delay_end(fig))
       ...
       if self.init_spectrum == True :
           ...
           self.data_processor.spectre = [0 for _ in range(4096)]
           self.data_processor1.spectre1 = [0 for _ in range(4096)]
           self.init_spectrum = False
           self.start_time = dt.datetime.now()
...
if __name__ == "__main__":
    ...
    app = Application()
    app.gui_manager.root.protocol(name="WM_DELETE_WINDOW", func = lambda: app.gui_manager.close_opal_kelly())
    app.run()
